...
(deftest test-has?
  ...
  (is (false? (sut/has? {:guch "nothing"} :article))
      "Expected false for a fragment with no :config.")
  (is (false? (sut/has? {:config #{:article}} ""))
      "Expected false for an option that's an empty string."))
